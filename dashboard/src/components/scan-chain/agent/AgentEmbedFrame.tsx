...
export function AgentEmbedFrame({
  ...
  const workspaceClass = "h-full min-h-[calc(100vh-200px)] overflow-hidden bg-[#090b12]";
  // Fill parent when it has height (recommendation tabs); keep min-h for scan-chain pages.
  const iframeClass = "h-full min-h-[calc(100vh-200px)] w-full border-0 bg-[#090b12]";
  ...
  if (isLive && (isLoading || probing) && !canShow) {
    return (
      <div className="flex h-full min-h-[280px] flex-col items-center justify-center bg-[#090b12]">
  ...
  return (
    <div
      className={`flex h-full min-h-[280px] flex-col items-center justify-center border border-dashed border-[#2D3748] px-6 text-center ${workspaceClass}`}
    >
