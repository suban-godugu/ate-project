...
async function getPatternRecommendationAgentHealth(): Promise<IntegrationHealth> {
  return apiFetch("/integrations/pattern-recommendation-agent/health", { auth: false });
}

export function PatternRecommendationAgent() {
  const live = isLiveApi();
  const [keepEmbedded, setKeepEmbedded] = useState(false);

  const patternRecHealth = useQuery({
    queryKey: ["integration", "pattern-recommendation-agent", "health"],
    queryFn: getPatternRecommendationAgentHealth,
    enabled: live,
    staleTime: 10_000,
  ...
  const health = patternRecHealth.data;
  const dashboardUrl = resolveAgentEmbedUrl(health?.embed_path, "patternRec");
  ...
  return (
    <AgentEmbedFrame
      ...
      isLive={live}
      isLoading={patternRecHealth.isLoading && !keepEmbedded}
      isFetching={patternRecHealth.isFetching}
      onRetry={() => patternRecHealth.refetch()}
      unavailableHint="Pattern Recommendation Agent is not running. Run start-agents.ps1, then open http://localhost:3000/dashboard and click Retry."
    />
  );
}
