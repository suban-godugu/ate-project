"""RBAC extension point and bounded throttling for FA-FR-009."""
...
async def fault_prediction_access_context(
    ...
    if role not in ALLOWED_ROLES:
        raise HTTPException(
            status_code=403,
            detail={
                "code": "FAULT_PREDICTION_ACCESS_DENIED",
...
async def fault_prediction_rate_limit(request: Request) -> None:
    ...
    if len(window) >= 30:
        raise HTTPException(
            status_code=429,
            detail={
                "code": "FAULT_PREDICTION_RATE_LIMITED",
