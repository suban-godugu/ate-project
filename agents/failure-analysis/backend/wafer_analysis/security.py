"""RBAC extension point and bounded throttling for FA-FR-008."""
...
async def wafer_access_context(
    ...
    if role not in ALLOWED_ROLES:
        raise HTTPException(
            status_code=403,
            detail={
                "code": "WAFER_ANALYSIS_ACCESS_DENIED",
...
async def wafer_analysis_rate_limit(request: Request) -> None:
    ...
    if len(window) >= 30:
        raise HTTPException(
            status_code=429,
            detail={
                "code": "WAFER_ANALYSIS_RATE_LIMITED",
