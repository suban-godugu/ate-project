"""Offline PA-ML-001-LOT training — never import from session request path."""
...
from ml.contracts import FEATURE_SCHEMA_VERSION_LOT, GENERATED_BY_LOT, MODEL_FAMILY_LOT
...
def train_pa_ml_001_lot(
    *,
    ...
    if not matrix:
        raise RuntimeError("No labeled LOT feature rows available for training.")
    x = np.asarray(matrix, dtype=np.float64)
    y, groups = _labels_and_groups(bundle)
    if len(set(y.tolist())) < 2:
        raise RuntimeError("Need both PASS and FAIL labels to train PA-ML-001-LOT.")
    ...
    card = {
        ...
        "notes": [
            "Experimental LOT-grain model trained offline from Analysis Session artifacts.",
            "Label = any FAIL in pattern×LOT; forbidden outcome aggregates excluded from features.",
        ],
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Train PA-ML-001-LOT Failure Risk offline (pattern × LOT)"
    )
