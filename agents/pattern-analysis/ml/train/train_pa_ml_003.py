"""Offline PA-ML-003 training (log grain) — never import from session request path."""
...
def train_pa_ml_003(
    *,
    ...
    if not matrix:
        raise RuntimeError("No labeled feature rows available for PA-ML-003 training.")
    x = np.asarray(matrix, dtype=np.float64)
    y, groups = labels_and_groups(bundle)
    if len(set(y.tolist())) < 2:
        raise RuntimeError("Need both PASS and FAIL labels to train PA-ML-003.")
    ...
    schema["feature_schema_version"] = FEATURE_SCHEMA_VERSION_003
    schema["feature_names"] = feature_names
    schema["grain"] = "pattern_x_source_log"
    schema["label"] = "latest_result_FAIL_equals_1"
    ...
    card: Dict[str, Any] = {
        ...
        "notes": [
            "Root-cause ranking model; outputs investigation priority for candidates only.",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Train PA-ML-003 Root-cause Ranking offline (log grain)"
    )
