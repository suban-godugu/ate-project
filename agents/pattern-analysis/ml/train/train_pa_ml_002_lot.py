"""Offline PA-ML-002-LOT training — never import from session request path."""
...
def train_pa_ml_002_lot(
    *,
    ...
    if not matrix:
        raise RuntimeError("No LOT feature rows available for PA-ML-002-LOT training.")
    ...
    schema["feature_schema_version"] = FEATURE_SCHEMA_VERSION_002_LOT
    ...
    card: Dict[str, Any] = {
        ...
        "notes": [
            "Unsupervised LOT-grain anomaly model; aggregates log executions per pattern×LOT.",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Train PA-ML-002 Anomaly offline (pattern × LOT grain)"
    )
